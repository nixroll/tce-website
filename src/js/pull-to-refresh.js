 ...
 *
 * 08.08: расширено на .body--light-top (/contact/, Header Line, см.
 * header-line.njk) — тот же класс бага, только без прозрачного Header:
 * пользователь на Mac Chrome при сильном флике трекпадом вверху видит
 * пустое поле БЕЗ вертикальных stroke, размером ЗАМЕТНО больше высоты
 * самого Header Line (то есть страница "улетает" от Header дальше, чем
 * пробовали компенсировать через ::before-расширение — тот приём
 * убран, см. style.css). Цвет там на деле уже верный (html:has(body.
 * body--light-top), совпадает с фоном секции) — жаловались на сам факт
 * такого большого зазора, не на цвет. Проще и надёжнее не пытаться
 * дорисовать контент в зазор, а не давать ему появиться вообще — тот
 * же проверенный приём, что и на /home/. */
(function () {
  var body = document.body;
  if (!body.classList.contains('body--header-overlay') && !body.classList.contains('body--light-top')) return;
