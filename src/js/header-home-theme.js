...
(function () {
  ...
  /* 05.08: пользователь заметил, что в самую первую секунду после
     загрузки (пока не начал скроллить) под iOS-статус-баром/чёлкой
     видно тёмное Hero-фото, а не белую полосу, хотя <meta theme-color>
     уже стоит "#ffffff" статически в HTML — белый фон появляется
     только когда реально начинается скролл. Причина — сам Safari:
     пока адресная строка в "развёрнутом" состоянии (страница ещё не
     скроллилась ни на пиксель), область под чёлкой показывает
     буквально содержимое страницы напрямую, а не подкрашивается по
     theme-color — переключение на "компактный" тулбар (после чего
     Safari уже красит эту область по meta-тегу) происходит только
     при первом реальном скролле. Стандартный обход — форсировать
     программный скролл на 1px сразу после загрузки, если страница ещё
     в самом верху: незаметно глазу (1px), но Safari сразу переходит в
     компактный режим и красит статус-бар правильно, не дожидаясь,
     пока пользователь сам начнёт скроллить. */
  if (window.scrollY === 0) {
