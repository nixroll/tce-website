...
(function () {
  ...
  /* 10.08, доп. подстраховка (пользователь, iOS: полоса статус-бара
     иногда остаётся серой после разворота скролла вниз→вверх на
     /about/ — см. подробный комментарий у .hero-l в style.2.css про
     нестабильность 100vh на iOS Safari во время скролла, основная
     причина устранена там через 100svh). На случай, если <meta
     theme-color> всё же на секунду разъедется с реальным цветом
     Header (сам класс Header — источник истины, синхронизация к нему
     всегда доступна через syncMetaThemeColor) — принудительно
     ресинхронизируем ещё раз, когда скролл ПОЛНОСТЬЮ остановился
     (scrollend, Safari 16.4+/iOS 16.4+ — если браузер событие не
     поддерживает, просто ничего не делает, никакого вреда). */
  if ('onscrollend' in window) {
    window.addEventListener('scrollend', syncMetaThemeColor, { passive: true });
  }
